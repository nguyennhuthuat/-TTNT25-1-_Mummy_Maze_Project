...
pygame.display.set_caption("Mummy Maze - Sign Up")

# --- Colors (Mystic Theme) ---
COLOR_WHITE = (235, 235, 235)
COLOR_GOLD = (255, 215, 0)
COLOR_SEMI_TRANSPARENT_WHITE = (255, 255, 255, 150)
COLOR_SEMI_TRANSPARENT_GOLD = (255, 215, 0, 180)
...
def save_user(fullname, username, password):
    ...
    try:
        ...
        print(f"Đăng ký thành công cho: {username}")
        return True
    except IOError as e:
...
def main():
    ...
    title_text = TITLE_FONT.render("TẠO TÀI KHOẢN", True, COLOR_WHITE)
    
    fullname_box = InputBox(SCREEN_WIDTH/2 - 225, SCREEN_HEIGHT/2 - 100, 450, 40, 'Họ và Tên')
    username_box = InputBox(SCREEN_WIDTH/2 - 225, SCREEN_HEIGHT/2 - 30, 450, 40, 'Tên Tài Khoản')
    password_box = InputBox(SCREEN_WIDTH/2 - 225, SCREEN_HEIGHT/2 + 40, 450, 40, 'Mật Khẩu', is_password=True)
    signup_button = Button(SCREEN_WIDTH/2 - 150, SCREEN_HEIGHT/2 + 120, 300, 60, "Đăng Ký")
    
    input_boxes = [fullname_box, username_box, password_box]
    ...
    while running:
        for event in pygame.event.get():
            ...
            if signup_button.handle_event(event):
                if not all([fullname_box.text, username_box.text, password_box.text]):
                    print("Vui lòng điền đầy đủ thông tin.")
                else:
                    if save_user(fullname_box.text, username_box.text, password_box.text):
                        pass
